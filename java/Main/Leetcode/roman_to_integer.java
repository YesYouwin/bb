...
class Solution {
    public int romanToInt(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (i < s.length() -1 && r.get(s.charAt(i)) < r.get(s.charAt(i + 1))) // && works like the word "and" 
                x -= r.get(s.charAt(i)); // basically making the number a negative integer. Say it's IV then I = -1 and V = 5 so -1 + 5 = 4
            }
            else {
                x += r.get(s.charAt(i));
            }
...
// 5 MS RUNTIME BABYYYYYYYYYYYYYYYY